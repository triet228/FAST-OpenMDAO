...
| `battery.py` | candidate | Effective cell capacity, selected current root, empirical cycling-aging SOH, one-step equivalent-circuit discharge/charge dynamics, and simple energy-based battery source weight converted; full discharge/charge histories, ground-charge loop, and detailed cell sizing components remain. |

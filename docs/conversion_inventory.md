...
| `data_struct.py` | complete-equation-scope | SpecProcessing power-to-weight and specific-energy unit conversion helper converted; remaining defaults, schema preparation, validation, regression default filling, and nested dictionary mutation are support/orchestration. |
...
| `specs.py` | support | AEA custom architecture plus LM100J_Hybrid custom architecture and operation split matrices converted; remaining aircraft and engine spec factories are dictionary assembly and preset data. |

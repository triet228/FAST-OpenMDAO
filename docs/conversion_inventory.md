...
| `battery.py` | candidate | Effective cell capacity, selected current root, and simple energy-based battery source weight converted; discharge, charge, aging, and detailed cell sizing components remain. |
...
| `engine.py` | candidate | Isentropic pressure, temperature, area-Mach, mass-flow, density, off-design nozzle Mach, fitted specific-heat, thermally perfect gamma update, local Reynolds, and local efficiency primitives converted; thermodynamic cycle, fan, compressor, turbine, and sizing components remain. |
...
| `mission.py` | candidate | `fast_openmdao.mission.FlightConditions`, cruise time-target distance conversion, Breguet cruise efficiency triplet, Breguet source-energy allocation, and initial source-energy remaining converted; segment, remaining Breguet cruise, and mission groups remain. |
...
| `propulsion.py` | candidate | Cable sizing weight, engine lapse, engine thrust requirement, power-flow propagation, safe component weight, efficiency selectors, and supplemental transmitter power converted; architecture, remaining sizing, fuel-use, and history components remain. |

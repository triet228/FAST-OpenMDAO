...
| `engine.py` | candidate | Isentropic pressure, temperature, area-Mach, mass-flow, density, off-design nozzle Mach, perfect-expansion nozzle flow, fitted specific-heat, inverse air-heat Newton solvers, thermally perfect gamma update, diffuser flow, burner fuel/exit-state flow, compressor/fan stage flow, turbine stage flow, local Reynolds, local efficiency, and simple off-design turbofan fuel-flow primitives converted; thermodynamic cycle, multi-stage compressor/fan/turbine wrappers, and sizing components remain. |

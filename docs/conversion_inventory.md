...
| `engine.py` | candidate | Isentropic pressure, temperature, area-Mach, mass-flow, density, off-design nozzle Mach, perfect-expansion nozzle flow, fitted specific-heat, inverse air-heat Newton solvers, thermally perfect gamma update, diffuser flow, burner fuel/exit-state flow, compressor/fan stage flow, turbine stage flow, local Reynolds, local efficiency, simple off-design turbofan fuel-flow primitives, and low-fidelity turboprop linear sizing converted; thermodynamic cycle, multi-stage compressor/fan/turbine wrappers, turbofan linear sizing, and nonlinear sizing components remain. |
